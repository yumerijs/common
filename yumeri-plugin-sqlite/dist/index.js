...
var SqliteDatabase = class _SqliteDatabase {
  ...
  static async create(dbPath) {
    ...
    return new _SqliteDatabase(driver);
  }
  getFieldDef(def) {
  ...
  buildColumnSql(field, def) {
    ...
    if (def.initial !== void 0) sql += ` DEFAULT ${JSON.stringify(def.initial)}`;
    if (def.autoIncrement) sql += " PRIMARY KEY AUTOINCREMENT";
    return sql;
  }
...
async function apply(ctx, config2) {
  const dbPath = path.join(process.cwd(), config2.path);
